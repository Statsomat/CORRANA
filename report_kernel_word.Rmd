...
Selection for the decimal character:
```{r, results="asis"}
cat(params$decimal)
```  
...
tryCatch({
  ...
  if (!exists("df_num") || ncol(df_num)==0L){
    cat("**Warning: No pairs of continuous variables detected for your data and selected options. No output generated.**", fill=TRUE)
  }
...
tryCatch({
  ...
  if (sum(rateunique_df) < 2L || nrow(df_num)<5L){
    cat("**Warning: No pairs of continuous variables detected or sample size smaller than 5. No output generated.**", fill=TRUE)
  }
    ...
    for (i in 1:ncol(combs)){
      ...
      # Consider bivariate continuous 
        ...
        tryCatch({     
          ...
          rm(list=setdiff(ls(), c("df2","output_num","params","rateunique_df", "df_num", "combs", "count_lin", "count_interpretp", "count_pearsoncorr","count_pformat",paste("refs",seq(1:16),sep=""))))
...
tryCatch({
  ...
  if (sum(rateunique_df) >= 2L & nrow(df_num)>=5){
    ...
    if (exists("count_lin")){
      if (count_lin>0) {
      cat("Preliminary assumptions used by the Statsomat app are based on statistical hypothesis tests performed at a 1%-5% type I error rate and calibrated with other test-specific rules. ", fill=TRUE)
      cat("\\newline",fill=TRUE)
      }
    }
