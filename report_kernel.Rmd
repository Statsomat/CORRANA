...
df_num <- df_num[,order(colnames(df_num)),drop=FALSE]
  ...
  stop(safeError("Outputs cannot be generated. Please check your data for consistency or try other upload options."))
}
...
Variables treated as numeric and continuous: 
```{r, results="asis"}
if (exists("df_num")){
  if (ncol(df_num)>0){
    rateunique_df <- sapply(df_num, function(col) (length(unique(col)) / nrow(df_num)) >= cutoffcont(nrow(df_num)))
    cat(sum(rateunique_df), fill=TRUE)
  } else {cat("0",fill=TRUE)}
}
```
...
if (exists("cols_catlarge")){
  if (sum(cols_catlarge) != 0L){
    cat("\n\n\\small Character columns with more than 10 unique values which are not evaluated in this app:") 
    cols <- names(which(cols_catlarge==1))
    kable(cols, col.names = "Column Names")
  }
}
```
...
tryCatch({
  ...
  if (!exists("df_num") || ncol(df_num)==0L){
    cat("**Warning: No pairs of continuous variables available. No output generated.**", fill=TRUE)
  }
...
tryCatch({
  ...
  if (sum(rateunique_df) < 2L || nrow(df_num)<5L){
    cat("**Warning: No pairs of continuous variables available or sample size smaller than 5. No output generated.**", fill=TRUE)
  }
...
tryCatch({
  ...
  if (sum(rateunique_df) >= 2L & nrow(df_num)>=5){
    cat("\n# Results for continuous vs continuous pairs", fill=TRUE)
    cat("Columns ordered alphabetically", fill=TRUE)
    output_num <- TRUE
  }}, error=function(e) {cat("")}
    ...
    for (i in 1:ncol(combs)){
      ...
      # Consider bivariate continuous 
        if (rateunique1 >= cutoffcont(nrow(df_num)) & rateunique2 >= cutoffcont(nrow(df_num))){
          ...
          title <- cbind(colnames(df_num)[k], "vs", colnames(df_num)[j])
          
          cat("\n## Column Numbers:",j,"and",k, fill=TRUE)
          
          print(kable(colnames(df_num)[c(j,k)],col.names="Column Names"))
      

          tryCatch({
            # Define check function for Pearson
            check <- linearity(df_num[,j],df_num[,k])
          }, error=function(e){cat("Error. This data pair cannot be analyzed. ", fill=TRUE)})     
          ...
          tryCatch({  
            if (check==TRUE) {
              output1 <- TRUE
              refs1 <- TRUE
              cat("If there is a relationship between the variables, then the Statsomat app assumes that it is linear. Therefore, the dependence between the variables is quantified by the Pearson correlation coefficient which is estimated to be:",round(pcorrelation,4),".", fill=TRUE)
            }
          }, error=function(e){cat("")}) 
          ...
          tryCatch({  
            if (check==TRUE & normality(df_num[,j],df_num[,k])==TRUE) {
              # Inverse Fisher CI
              fisherci <- cor.test(df_num[,j],df_num[,k], method="pearson")
              output2 <- TRUE
              refs2 <- TRUE
              cat("The Statsomat app assumes no great departure from the normal distribution. Therefore, it computes an asymptotic 95% confidence interval for the Pearson correlation coefficient, based on the Fisher's Z transform: (", round(fisherci$conf.int[1],4),round(fisherci$conf.int[2],4), ").", fill=TRUE)
          ...
          tryCatch({  
            if (check==TRUE & normality(df_num[,j],df_num[,k])==FALSE & nrow(df_num) < 800) {
              ...
              cat("The Statsomat app assumes that the data is not normally distributed. Therefore, it computes a 95% confidence interval for the Pearson correlation coefficient based on bootstrapping: (",
              ...
              if ((nrow(df_num) < 8)) {
                cat("Warning: The data sample size may be too small for a reliable bootstrap confidence interval.", fill=TRUE)
                cat("\\newline",fill=TRUE)
                check_nbci <- TRUE
              }
            }
          ...
          tryCatch({  
            if (check==TRUE & normality(df_num[,j],df_num[,k])==FALSE){
              output4 <- TRUE
              refs4 <- TRUE
              cat("The Statsomat app assumes that the data is not normally distributed. Therefore, it evaluates the association between the variables also by using the Spearman rank correlation coefficient which is estimated to be:",round(spcorrelation,4),". ", fill=TRUE)
            }
          }, error=function(e){cat("")}) 
          ...
          tryCatch({  
            if (check==FALSE & checkm==TRUE) {
              output5 <- TRUE
              refs5 <- TRUE
              cat("If there is a relationship between the variables, then the Statsomat app assumes that it is best described by a monotonic but not necessarily linear function. In this case the Pearson correlation coefficient may not be the optimal measure of dependency. The association between the variables is evaluated by the Spearman rank correlation coefficient, which is estimated to be: ",round(spcorrelation,4),". ")
            }
          }, error=function(e){cat("")}) 
          ...
          tryCatch({ 
            ...
            if (check==FALSE & checkm==FALSE){
              ...
              tryCatch({ 
                ...
                micval <- minerva::mine(df_num[,j],df_num[,k], na.rm=TRUE)$MIC
                
                cat("The Statosmat app uses another measure(s) to quantify the dependence between the variables. ", fill=TRUE)
                cat("\\newline",fill=TRUE)
                ...
                if (mic@p_value > 0.05){ 
                  ...
                  cat("\\newline",fill=TRUE)
                  cat("Background: The Maximal Information Coefficient is related to the relationship strenght and it can be interpreted as a correlation measure. It is symmetric and it ranges in [0,1], where it tends to 0 for statistically independent data and it approaches 1 in probability for noiseless functional relationships. ", fill=TRUE)
                  cat("\\newline",fill=TRUE)
                }
                if (mic@p_value <= 0.05){ 
              ...
              tryCatch({ 
                ...
                if (dist$p.value > 0.05){ 
                  ...
                  cat("The statistical test of a zero distance correlation leads to a p-value of: ",round(dist$p.value,3),". Therefore, the distance correlation is not statistically significant from 0 at a type I error rate of 5%. By using the distance correlation measure, the Statsomat app cannot identify a dependence between the variables. ", fill=TRUE)
                  cat("\\newline",fill=TRUE)
                ...
                if (dist$p.value <= 0.05){ 
                  cat("The distance correlation between the variables is ",round(dist$statistic,3),".", fill=TRUE)     
                  cat("The statistical test of a zero distance correlation leads to a p-value of: ",round(dist$p.value,3), ". Therefore, the distance correlation is statistically significant from 0 at a type I error rate of 5%. By using this measure, the Statsomat app identifies a statistically significant dependence between the variables. ", fill=TRUE)
        ...
        tryCatch({     
          ...
          if ((check==TRUE & normality(df_num[,j],df_num[,k])==FALSE & kentest$p.value <= 0.1) || (check==FALSE & checkm==TRUE & kentest$p.value <= 0.1)) {
            
            if (min(length(unique(df_num[,j])), length(unique(df_num[,k]))) < nrow(df_num)){
            ...
            if ((nrow(df_num) < 800 & kentest$p.value <= 0.1 & spearmanci2$p.value>0.1) || (nrow(df_num) < 800 & kentest$p.value > 0.1 & spearmanci2$p.value<=0.1)){
              cat("Note: For this sample size, the Statsomat app prefers the Kendall's tau to the Spearman hypothesis test. ", fill=TRUE)
              cat("\\newline",fill=TRUE)
            }
          }
        }, error=function(e){cat("")}) 
          ...
          tryCatch({      
            ...
            if (check==TRUE) {
              ...
              if (normality2(residuals(lm(df_num[,k] ~ df_num[,j])))==TRUE){
                outliers <- outlierTest(lm(df_num[,k] ~ df_num[,j]))
                outliers_sign <- outliers$bonf.p[outliers$bonf.p<0.01]
              }
              
              if (normality2(residuals(lm(df_num[,j] ~ df_num[,k])))==TRUE){
                outliers2 <- outlierTest(lm(df_num[,j] ~ df_num[,k]))
                outliers_sign2 <- outliers2$bonf.p[outliers2$bonf.p<0.01]
              }
              
              if (length(outliers_sign) > 0L || length(outliers_sign2) > 0L) {
           ...
           tryCatch({ 
            if (check==TRUE & output12==FALSE){
              if (knnoutlier(df_num[,j],df_num[,k])==TRUE || knnoutlier(df_num[,k],df_num[,j])==TRUE){
                cat("Warning: The Statsomat app detected possible outliers for this variable pair. Outliers may have a negative effect on the reported results related to the Pearson correlation coefficient. Please check your data before uploading to Statsomat or contact the classical statistical consultancy at support@statsomat.com.", fill=TRUE)
                cat("\\newline",fill=TRUE)
              }
             }
           ...
           tryCatch({ 
            ...
            # Breusch-Godfrey test, autocorrelation existent if check is false
             if (autocorr(df_num[,j],df_num[,k])==FALSE){
           ...
           if (check==FALSE & checkm==TRUE & output13==FALSE) {
            # Breusch-Godfrey test on ranks 
             if (autocorr(rank(df_num[,j]),rank(df_num[,k])) == FALSE){
          ...
          tryCatch({ 
             # Small linear correlation - maybe other sort of dependence? 
              text <- "Warning: Other measures of dependence besides the classical coefficients of correlation e.g. the Distance Correlation or the Maximal Information Coefficient may indicate a significant relationship between the variables. For more help, please contact the classical statistical consultancy at support@statsomat.com."
              
              if (check==TRUE){
                if (abs(pcorrelation)<0.2 & output14==FALSE) {
                  if (dependence(df_num[,j],df_num[,k])==TRUE) {
                  refs14 <- TRUE
                  output14 <- TRUE
                  cat(text, fill=TRUE)
                  cat("\\newline",fill=TRUE)
          ...
          # Sample size warning < 25
            if (nrow(df_num) < 25) {
                cat("Warning: The sample size may be too small to guarantee reliable confidence intervals and an acceptable power of the statistical tests implied in the results.", fill=TRUE)
                cat("\\newline",fill=TRUE)
            }
          ...
          tryCatch({  
            cat("",fill=TRUE)
            scatterplotMatrix(~df_num[,j]+df_num[,k], smooth=FALSE, var.labels=colnames(df_num)[c(j,k)], main="Enhanced Scatterplots", col = "#2fa42d")
            output15 <- TRUE
            refs15 <- TRUE
            cat("\n\n\\pagebreak\n")
          }, error=function(e){cat("")}) 
          ...
          rm(list=setdiff(ls(), c("output_num","params","rateunique_df", "df_num", "combs", "count_lin", "count_interpretp",paste("refs",seq(1:16),sep=""))))
  ...
  if (sum(rateunique_df) >= 2L & nrow(df_num)>=5){
    ...
    if (nrow(df_num)<5000){
      if ((refs2==TRUE || refs3==TRUE || refs4==TRUE)){
        cat("The assumption of normality was checked by using the Shapiro-Wilk normality test with the shapiro.test function.", fill=TRUE)
        cat("\\newline",fill=TRUE)
      }
    }
    ...
    if (nrow(df_num)>=5000){
      if ((refs2==TRUE || refs3==TRUE || refs4==TRUE)){
        cat("The assumption of normality was checked by using the package nortest [@nortest] and the Anderson-Darling normality with the ad.test function.", fill=TRUE)
        cat("\\newline",fill=TRUE)
      }
    }
    ...
    if (refs13==TRUE){
     ...
     # cat("An auxiliary check of serial correlation was done using the lmtest package [@lmtest] and the Breusch-Godfrey test with the # the bgtest function.", fill=TRUE)
    #  cat("\\newline",fill=TRUE)
    }
    ...
    if (exists("count_lin")){
      if (count_lin>0) {
      cat("Preliminary assumptions are based on statistical hypothesis tests performed at a 1%-5% type I error rate and calibrated with other test-specific rules. ", fill=TRUE)
      cat("\\newline",fill=TRUE)
      }
    }
