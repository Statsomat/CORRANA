...
if (union(params$vars1,colnames(df)) != colnames(df)){
...
for (i in 1:ncol(combs)){
  ...
  # Consider pair only if bivariate continuous 
    if (rateunique1 >= cutoffcont(length(na.omit(df_num[,j]))) && rateunique2 >= cutoffcont(length(na.omit(df_num[,k])))){
      ...
      print(kable(colnames(df_num)[c(j,k)],col.names="Column Names"))
        ...
        # Only if complete obs available
        if (nrow(temp2)>0){
          ...
          tryCatch({  
            if (check==TRUE && normality(df_num[,j],df_num[,k])==FALSE && nrow(temp3) < 800) {
              # Bootstrap CI
              output3 <- TRUE
              refs3 <- TRUE
        ...
        tryCatch({     
          ...
          if ((check==TRUE && normality(df_num[,j],df_num[,k])==FALSE && kentest$p.value <= 0.1) || (check==FALSE && checkm==TRUE && kentest$p.value <= 0.1)) {
            ...
            if ((nrow(temp3) < 800 && kentest$p.value <= 0.1 && spearmanci2$p.value>0.1) || (nrow(temp3) < 800 && kentest$p.value > 0.1 && spearmanci2$p.value<=0.1)){
          ...
          tryCatch({ 
             # Small linear correlation - maybe other sort of dependence? 
              text <- "Warning: Other measures of dependence besides the classical coefficients of correlation e.g. the Distance Correlation or the Maximal Information Coefficient could indicate a significant relationship between the variables. For more help, please contact the classical statistical consultancy at support@statsomat.com."
  ...
  } else { # and for non empty complete obs 
      
    cat("No complete cases available for this variable pair. ")
    cat("\\newline",fill=TRUE)    
      
}}} # end for each pair with bivariate continuous 
```  
