...
cat("Variables suitable for this app: ", fill=TRUE)
if (exists("df_num")){
...
if (exists("df_num")){
  if (sum(rateunique_df==TRUE) < ncol(df_num)){ 
    cat("Variables which are possibly continuous or ordinal: ", fill=TRUE)
    cat(sum(rateunique_df==FALSE),fill=TRUE)
    kable(cols_necontinuous, col.names = "Possibly continuous or ordinal")
  }
}

cat("Variables considered discrete (not further considered by this app): ", fill=TRUE)
if (exists("df_char")){
 cat(ncol(df_char))
 cat("\\newline",fill=TRUE) 
 cat("\\newline",fill=TRUE) 
...
# Go for next chunk
if (ncol(df_cont)>=2 && nrow(df_cont)>=5){
...
if (ncol(df_cont)>=2 && nrow(df_cont)>=5 && sum(rateunique_df==TRUE) < ncol(df_num)){ 
    cat("**Warning: This app is especially designed to analyze and interpret correlations or associations between independent and identically, approximately continuously distributed random variables measured in pairs. **" )
  cat("\\newline",fill=TRUE) 
  cat("\\newline",fill=TRUE) 
}
        ...
        tryCatch({     
          ...
          if ((check==TRUE && normality(df_cont[,j],df_cont[,k])==FALSE && kentest$p.value <= 0.1) || (check==FALSE && checkm==TRUE && kentest$p.value <= 0.1)) {
            ...
            if (min(length(unique(na.omit(df_cont[,j]))), length(unique(na.omit(df_cont[,k])))) < nrow(datapaircomplete)){
              ...
              cat("Note: Since there exist ties in the data, the p-value for the Kendall's tau hypothesis tests is only approximate. Moreover, in this case the Statsomat app recommends the Kendall's tau measure of association instead of the Spearman correlation coefficient. ", fill=TRUE)
              cat("\\newline",fill=TRUE)
            }
            if (nrow(datapaircomplete) < 800 && spearmanci2$p.value>0.1){
