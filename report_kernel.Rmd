...
# Go for next chunk
if (ncol(df_cont)>=2 && nrow(df_cont)>=7){
...
if (ncol(df_cont)>=2 && nrow(df_cont)>=7 && sum(rateunique_df==TRUE) < ncol(df_num)){ 
    cat("**Warning: This app is designed to analyze and interpret correlations or associations between independent and identically, approximately continuously distributed random variables measured in pairs. Variables for which these assumptions are questionable are not considered by the app. **" )
  cat("\\newline",fill=TRUE) 
  cat("\\newline",fill=TRUE) 
}
