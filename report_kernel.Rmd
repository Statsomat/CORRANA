...
tryCatch({
  ...
  if (!exists("df_num") || ncol(df_num)==0L){
    cat("**No pairs of continuous variables detected for your data and selected options. No output generated.**", fill=TRUE)
  }
...
tryCatch({
  ...
  if (sum(rateunique_df) < 2L || nrow(df_num)<5L){
    cat("**No pairs of continuous variables detected or sample size smaller than 5. No output generated.**", fill=TRUE)
  }
      ...
      # Consider bivariate continuous 
        if (rateunique1 >= cutoffcont(length(na.omit(df_num[,j]))) & rateunique2 >= cutoffcont(length(na.omit(df_num[,k])))){
          ...
          tryCatch({  
            if (check==TRUE & normality(df_num[,j],df_num[,k])==FALSE & nrow(df_num) < 800) {
              ...
              if ((nrow(temp2) < 10)) {
                cat("Warning: The sample size may be too small for a reliable bootstrap confidence interval.", fill=TRUE)
                cat("\\newline",fill=TRUE)
                check_nbci <- TRUE
              }
            }
          ...
          tryCatch({  
            if (check==FALSE & checkm==TRUE) {
              output5 <- TRUE
              refs5 <- TRUE
              cat("If there is a relationship between the variables, then the Statsomat app assumes that it is described by a monotonic but not necessarily linear function. In this case the Pearson correlation coefficient may not be the optimal measure of dependency. The association between the variables is evaluated by the Spearman rank correlation coefficient, which is estimated to be: ",round(spcorrelation,4),". ")
            }
          }, error=function(e){cat("")}) 
        ...
        tryCatch({     
          ...
          if ((check==TRUE & normality(df_num[,j],df_num[,k])==FALSE & kentest$p.value <= 0.1) || (check==FALSE & checkm==TRUE & kentest$p.value <= 0.1)) {
            ...
            if (min(length(unique(na.omit(df_num[,j]))), length(unique(na.omit(df_num[,k])))) < nrow(temp2)){
            ...
            if ((nrow(temp2) < 800 & kentest$p.value <= 0.1 & spearmanci2$p.value>0.1) || (nrow(temp2) < 800 & kentest$p.value > 0.1 & spearmanci2$p.value<=0.1)){
          ...
          # Sample size warning < 25
            if (nrow(temp2) < 25) {
                cat("Warning: The sample size may be too small to guarantee correct decisions made by the Statsomat app, an acceptable power of the statistical tests and reliable confidence intervals.", fill=TRUE)
                cat("\\newline",fill=TRUE)
            }
