...
if (exists("df_cont") && ncol(df_cont)>=2 && nrow(df_cont)>=7){
  ...
  if (length(which(complete_rate < 0.5)) != 0L){
    cat("\\textcolor{blue}{Warning: These variables have more than 50% missing values: }")
  ...
  # Consider pair only if complete cases >=7 and unique >=5
    if (min(unique1,unique2) >= 5 && nrow(datapaircomplete)>=7){
          ...
          if (min(length(unique(na.omit(df_cont[,j]))), length(unique(na.omit(df_cont[,k])))) < nrow(datapaircomplete)){
             cat("\\textcolor{blue}{Warning: There are some ties in this variable pair. Despite the ties found, the app consideres each variable to be independent and identically, approximately continuously distributed. Please make sure that your experimental design supports the assumptions of this app. }", fill=TRUE)
             cat("\\newline",fill=TRUE)
          }
    ...
    if (nrow(datapaircomplete)<7){  
      cat("Result for this pair: Sample size smaller than 7. No output generated. ")
      cat("\\newline",fill=TRUE)
      cat("\\newline",fill=TRUE)
    } else {
      cat("Result for this pair: Less than 5 distinct values for one variable available. No output generated. ")
      cat("\\newline",fill=TRUE) 
      cat("\\newline",fill=TRUE)
    }
